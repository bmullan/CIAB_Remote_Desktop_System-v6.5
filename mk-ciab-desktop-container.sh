...
echo "   configured with a \"ciab-share\" folder on the  User\'s Container \"Desktop\".   In that folder is the User\'s"
...
echo
echo " During this installation the following Desktop Envrionment (DE) choices are available to select:"
echo
echo "     - KDE Plasma"
echo "     - MATE"
echo "     - Gnome"
...
#	sudo chown ${USER_NAME}:${USER_NAME} ${SHARED_FOLDER}
#fi
...
#================
# NOTE: may add /multimedia:sys:alsa

cat > /home/${USER_NAME}/${CN_NAME}.sh <<END
#!/bin/bash
xfreerdp /cert-ignore +gfx /rfx /rfx-mode:video /video /bpp:32 /dynamic-resolution /home-drive +clipboard /sound:sys:pulse /v:$CN_IP
END
